...
const Player = struct { position: rl.Vector2, size: rl.Vector2, speed: rl.Vector2 };

const World = struct { player: Player };
...
pub fn main() anyerror!void {
    var world = initWorld(screenHeight, screenWidth);
...
fn initWorld(height: i32, width: i32) World {
    const playerXOffset = playerSize.x / 2;
    const playerYOffset = playerSize.y / 2;
    return World{ .player = init_player((@as(f32, @floatFromInt(width)) / 2.0) - playerXOffset, (@as(f32, @floatFromInt(height)) / 1.25) - playerYOffset) };
}
